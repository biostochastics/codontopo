...
#block(
  ...
  The standard genetic code is structured to reduce the impact of point mutations, but the robustness of this property across physicochemical metrics, naturally variant codes, and codon-reassignment mechanisms remains incompletely quantified. We embed the 64 codons in $"GF"(2)^6$, representing the hypercube $Q_6$ as a coordinate-dependent subgraph of the encoding-independent single-nucleotide mutation graph $H(3,4)$. This decomposition supports continuous interpolation between $Q_6$ and the full mutation graph and joint analysis of physicochemical error minimization and codon-family topology. Under a block-preserving null ($n = $10,000), the standard code is significantly low-cost across four conceptually distinct amino-acid distance metrics---Grantham, Miyata, Woese polar requirement, and Kyte--Doolittle hydropathy ($p lt.eq 0.006$ for all four)---closing the standing objection that earlier optimality results depended on the choice of metric. The signal strengthens monotonically as diagonal-edge weight increases from $Q_6$ toward $H(3,4)$. Across the 27 NCBI variant translation tables, near-optimality is broadly preserved: among the 12 codes differing from the standard by at least three reassignments, 11 retain top-5% placement after BH--FDR correction, with yeast mitochondrial code as the sole marginal exception; near-standard codes are also significant but read primarily as confirmations of standard-code geometry. Natural codon reassignments rarely break codon-family connectivity: under $H(3,4)$, only #tk43.observed_breaks of #tk43.observed_total observed events increase amino-acid codon-family disconnectedness, compared with #str(calc.round(tk43.rate_possible * 100, digits: 0))% of 1,280 candidate single-codon relabelings (RR #str(calc.round(tk43.risk_ratio, digits: 2)), permutation $p lt.eq 10^(-4)$). The cross-lineage signal rests on the absence of breakers across most clades---22 of 28 events do not break topology, distributed across many lineages---rather than on repeated independent breaking-then-failing: 4 of the 6 breakers come from a single mitochondrial lineage (yeast, NCBI table 3), and the breaker subset is underpowered for cross-clade structure. The depletion is robust to alternative topology definitions, phylogenetic clade exclusions, and base-to-bit encodings. Crucially, conditional-logit decomposition shows that topology avoidance and local physicochemical cost provide *complementary*, only weakly correlated explanatory signal ($r_s = #str(calc.round(cl.phys_topo_rho, digits: 2))$): each retains substantial $Delta$AICc after the other is included, with neither reducible to the other. A heuristic tRNA-distance proxy does not improve fit, and several variant-code lineages show suggestive enrichment of tRNA genes for reassigned amino acids. Retrospective reanalysis of nine published genome-recoding datasets is consistent with---but does not establish---a working hypothesis in which codon-family topology operates over evolutionary timescales rather than acute cellular fitness: Syn61 tolerated 18,218 boundary-crossing serine swaps with 99.96% success, yet the same boundary-crossing class is #str(calc.round(tk43.depletion_fold, digits: 1))-fold depleted across natural code evolution. The conceptual upgrade over single-axis treatments is the second axis itself: code evolution is constrained simultaneously by physicochemical smoothness (preserving protein function under mistranslation) and by codon-family topological integrity (preserving the connectivity that decoding machinery services), and these two constraints are partly independent.
...
Following #cite(<sengupta2007>, form: "prose"), we performed clade-exclusion sensitivity analysis, iteratively removing each major taxonomic group (all ciliates, all metazoan mitochondria, all CUG-clade yeasts, etc.) and retesting. The depletion is highly significant in every regime ($p < 10^(-5)$), but the structure of the result warrants honest reading: the topology-breaking events themselves are not pan-taxonomic. Four of the 6 breakers under $H(3,4)$ come from a single clade (yeast mitochondrial, table 3; all 4 of its de-duplicated reassignments are breakers); excluding it leaves $2 / 24 = 8.3$% breakers, with hypergeometric $p$ moving from #sci(tq6.hypergeom_p) to $3.6 times 10^(-11)$. The clade-exclusion robustness is thus a denominator effect---removing clades that contributed zero or one breaker each leaves the breakage rate essentially unchanged---rather than evidence of repeated breaking-then-failing across many lineages. The pan-taxonomic signal is therefore the *avoidance* of topology-breaking moves, supported by 22 of 28 events distributed across many lineages; the cross-clade structure of the breaker set itself is underpowered.

The yeast-mitochondrial concentration is informative rather than embarrassing. Table 3 is the high-$k$ outlier of the 27-table set (the most extensively reassigned code in the registry) and is the only variant code requiring acquisition of a novel tRNA from a different parent (tRNA#super[Thr] derived from tRNA#super[His]; @su2011). Its surfacing as the marginal case in every $k$-aggregating diagnostic---the per-table optimality analysis (§3.3), the conditional-logit case with the lowest observed-move percentile (§3.5), and the topology-breaker concentration here---is consistent with the working interpretation that topology-breaking is not impossible but *expensive*, and is concentrated in lineages where other dimensions of the translation system are already unusually perturbed. The three diagnostics are not independent (a shared $k$-magnitude effect links them by construction), so we do not treat the convergence as multi-test confirmation of any specific selectionist mechanism.
...
Two partly independent constraints govern code evolution within that region---not one---and the conceptual upgrade over single-axis treatments is the second. Persistent natural reassignments preferentially enter neighborhoods of lower local Grantham mismatch (preserving protein function under mistranslation), and they avoid fragmenting amino-acid codon-family connectivity (preserving the connected substrate that existing decoding machinery can service without new molecular hardware). Topology-breaking moves are approximately #str(calc.round(tk43.depletion_fold, digits: 1))-fold depleted under the encoding-independent $H(3,4)$ adjacency, and a conditional-logit decomposition shows that topology adds explanatory value beyond physicochemistry, and vice versa, by margins well above the conventional $Delta$AICc$>10$ reference ($Delta"AICc" gt.eq #str(calc.round(calc.min(cl.model_fits.M1_phys.aicc - cl.model_fits.M3_phys_topo.aicc, cl.model_fits.M2_topo.aicc - cl.model_fits.M3_phys_topo.aicc), digits: 0))$ under $Q_6$ topology; $Delta"AICc" gt.eq #str(calc.round(calc.min(cl.at("encoding_robustness", default: (:)).at("delta_aicc_M1_to_M3_k43", default: 0), cl.at("encoding_robustness", default: (:)).at("delta_aicc_M2k43_to_M3k43", default: 0)), digits: 0))$ under encoding-independent $H(3,4)$ topology). The two axes are only weakly correlated across the candidate landscape ($r_s = #str(calc.round(cl.phys_topo_rho, digits: 2))$); neither is a redundant restatement of the other. Because $Q_6$ adjacency is encoding-dependent (8 of 24 base-to-bit bijections give no $Q_6$ topology depletion in the candidate landscape), the survival of M3 dominance under $H(3,4)$ rules out a coordinate artifact: the topology constraint is a property of nucleotide-level codon-family connectivity, not of the binary representation. The topology coefficient is robust to all seven phylogenetic clade exclusions ($Delta"AICc"("M1" arrow.r "M3") gt.eq 49$ in the worst regime), and a posterior-predictive simulation under M3 reproduces the observed topology-breaking rate ($p = 0.60$). We make no causal claim about mechanism---tRNA cross-recognition range, ribosomal decoding geometry, and other facets of translational architecture remain plausible candidates not separately modeled here---and cross-sectional NCBI data cannot distinguish selection against attempting topology-breaking moves from selection against lineages that attempted them. Several variant-code lineages do show suggestive tRNA-gene enrichment for the reassigned amino acid (worst-case MIS Stouffer $p = #str(calc.round(trna.mis_worst_p, digits: 3))$ across 24 pairings from 18 tRNAscan-SE-verified genomes), consistent with compensatory accommodation; the topology-breaking-restricted subset alone is underpowered ($n = 4$).

A retrospective cross-study reanalysis of eight published genome-recoding experiments ($>$217,000 codon-level observations) sharpens the scope of the topology constraint. Family-boundary crossing is a real structural distinction over evolutionary timescales but did *not* predict acute transcriptomic perturbation in the Syn57 contrast, where Syn61 had earlier tolerated 18,218 boundary-crossing serine swaps with 99.96% success @fredens2019. The most parsimonious reading is that the topology constraint is not a property of immediate cellular fitness---a modern _E. coli_ with an already-optimized translation apparatus absorbs boundary-crossing without measurable burden---but of *evolutionary trajectories*: over the long stretches during which a codon family gradually accumulates the tRNA repertoire that services it, fragmenting that family creates a window of decoding fragility long enough for selection to remove the lineage. Synthetic biology bypasses that window by providing the decoding machinery up-front. This places codon-space topology in an unusual category: a constraint on how genetic codes can change rather than on how they currently function, path-dependent rather than instantaneous. Local neighborhood mismatch, by contrast, aligned with established recoding-burden covariates in the topology-fixed Napolitano arginine setting, and realized design deviations from Syn57 were dominated by mutational accessibility rather than systematic optimization of mismatch cost. The three-layer integration of these descriptors (evolutionary topology, fine-scale recoding burden, accessibility-driven escape) is exploratory: each layer-claim rests on a single retrospective contrast, and the partition was not pre-specified (§3.9.2). We frame it as a hypothesis suitable for prospective follow-up rather than a confirmed model.

The framework's algebraic ambitions remain bounded. The KRAS--Fano clinical prediction, Serine distance-4 invariant, PSL(2,7) symmetry, and holomorphic-embedding claims were tested and rejected (@tbl:claims): the binary representation captures the codon-to-amino-acid mapping, not the dynamics of mutation itself, and several "invariants" of the representation turned out to be encoding artifacts. The $"GF"(2)^6$ machinery contributes two genuine analytical tools---the $rho$-sweep interpolating between $Q_6$ and $H(3,4)$ and the $epsilon$-filtration on codon-family connectivity---plus a third, the encoding-sensitivity audit, that is best understood as methodological hygiene the framework owes itself rather than as a contribution from it. What survives is an encoding-independent picture of the standard code as a low-cost coloring of $H(3,4)$ whose persistent reassignment trajectories respect two partly independent constraints---the physicochemistry of the substituted amino acid and the connectivity of the codon family it joins. Whether the topology axis is a direct selection target or a proxy for some specific decoding-architecture variable is open; that there are two axes, and that single-axis treatments leave the second one's signal unaccounted for, is the working model to beat.
