...
#block(
  ...
  The standard genetic code is structured to reduce the impact of point mutations, but the robustness of this property across physicochemical metrics, naturally variant codes, and codon-reassignment mechanisms remains incompletely quantified. We embed the 64 codons in $"GF"(2)^6$, representing the hypercube $Q_6$ as a coordinate-dependent subgraph of the encoding-independent single-nucleotide mutation graph $H(3,4)$. This framework allows interpolation between $Q_6$ and the full mutation graph and permits simultaneous analysis of physicochemical error minimization and codon-family topology. Under a block-preserving null model ($n = $10,000), the standard code is significantly low-cost across four amino-acid distance metrics: Grantham, Miyata, Woese polar requirement, and Kyte--Doolittle hydropathy ($p lt.eq 0.006$ for all metrics). The error-minimization signal strengthens monotonically as diagonal-edge weight increases from $Q_6$ toward $H(3,4)$. Across NCBI variant translation tables, this near-optimality is broadly preserved: among the 12 codes differing from the standard code by at least three reassignments, 11 retain top-5% placement after BH--FDR correction, with yeast mitochondrial code as the sole marginal exception; near-standard codes are also significant but are interpreted primarily as confirmations of standard-code geometry. Natural codon reassignments are strongly depleted for topology-breaking moves. Under $H(3,4)$, only #tk43.observed_breaks of #tk43.observed_total observed events increase amino-acid codon-family disconnectedness, compared with #str(calc.round(tk43.rate_possible * 100, digits: 0))% of 1,280 candidate single-codon relabelings (RR #str(calc.round(tk43.risk_ratio, digits: 2)), permutation $p lt.eq 10^(-4)$). This depletion is robust to alternative topology definitions, phylogenetic clade exclusions, and base-to-bit encodings, motivating $H(3,4)$ as the primary topology graph. Conditional-logit models show that topology avoidance and local physicochemical cost provide complementary explanatory signal, whereas a heuristic tRNA-distance proxy does not improve fit. Several variant-code lineages show suggestive enrichment of tRNA genes for reassigned amino acids, although topology-breaking subsets remain underpowered. Retrospective analysis of published genome-recoding datasets suggests that codon-family topology constrains natural code evolution, whereas acute synthetic-recoding burden is better captured by local neighborhood mismatch and mutational accessibility. Together, these results support a two-axis model of genetic-code evolution in which physicochemical smoothness and codon-family topological integrity impose partly independent constraints.
...
To delimit the framework's algebraic scope, we examine four conjectures that have been previously circulated in informal methodological notes and presentations associated with this project (one author's prior unpublished technical note #cite(<clayworth2026>); the relevant claim statements are quoted verbatim where they are tested)---a Serine distance-4 invariance claim, PSL(2,7) symmetry, a holomorphic embedding extending a character of $"GF"(8)^*$, and a KRAS--Fano clinical prediction---and report their falsification or rejection transparently (@sec:res-negatives and Supplementary Note S1). Including these tests is intended to delimit scope, not to establish straw targets: each conjecture had a coherent algebraic motivation, and showing where the framework does *not* extend is part of giving an honest account of where it does.
...
Following the phylogenetic-distribution analysis of mitochondrial reassignment events by #cite(<sengupta2007>, form: "prose"), we performed clade-exclusion sensitivity analysis, iteratively removing each major taxonomic group (all ciliates, all metazoan mitochondria, all CUG-clade yeasts, etc.) and retesting. The depletion is highly significant in every regime ($p < 10^(-5)$). However, the structure of this finding warrants honest reading: the topology-breaking events themselves are not pan-taxonomic. Of the 6 topology-breaking reassignments under $H(3,4)$, 4 are concentrated in yeast mitochondrial (table 3), and excluding that single clade *strengthens* the depletion (observed rate drops from #str(calc.round(tq6.rate_observed * 100, digits: 1))% to 8.3%; hypergeometric $p$ from #sci(tq6.hypergeom_p) to $3.6 times 10^(-11)$). The clade-exclusion robustness is therefore a denominator effect---removing clades that contributed zero or one topology-breaking event each leaves the breakage rate essentially unchanged---rather than a numerator effect that would demonstrate breaking-then-failing across multiple lineages. The honest reading is: the *avoidance* of topology-breaking moves is pan-taxonomic, supported by 22 of 28 events that did not break topology distributed across many clades; the *attempts* to break topology are concentrated in one mitochondrial code, and with only 6 breakers the cross-clade structure of the breaker set is genuinely underpowered.

This concentration is itself informative. Yeast mitochondrial is also the marginal outlier in the per-table optimality analysis (§3.3), the conditional-logit case with the lowest observed-move percentile (§3.5), and the only variant code requiring acquisition of a novel tRNA derived from a different parent (tRNA#super[Thr] from tRNA#super[His]; @su2011). Under the null hypothesis that table 3 is just one of 27 tables drawn at random, the prior probability that one specific table converges across three independent analyses is on the order of $1/27^2 approx 1/700$, so this convergence is supporting evidence rather than coincidence. We treat it as evidence that yeast mitochondrial occupies a distinct position in the codon-reassignment landscape---one where multiple constraints (optimality, topology, mechanism) line up in the same direction---rather than as evidence for or against any specific selectionist mechanism.
...
#figure(
  table(
    ...
    [_E. aediculatus_], [Variant], [10], [GCA_030463445.1], [80], [3 Cys],
    [_E. amieti_], [Variant], [10], [GCA_048569255.1], [120], [4 Cys],
    [_E. focardii_], [Variant], [10], [GCA_001880345.2], [62], [1 Cys],
    [_E. parawoodruffi_], [Variant], [10], [GCA_021440025.1], [149], [5 Cys],
    [_E. weissei_], [Variant], [10], [GCA_021440005.1], [495], [17 Cys],
    [_E. woodruffi_], [Variant], [10], [GCA_027382605.1], [83], [2 Cys],
...
A retrospective, exploratory cross-study reanalysis of eight published genome-recoding experiments ($>$217,000 codon-level observations) suggests, but does not prove, that the same codon-space descriptors operate at different biological layers in evolutionary versus engineering contexts. Family-boundary crossing is a real structural distinction over evolutionary timescales but did not predict acute transcriptomic perturbation in the Syn57 contrast; local neighborhood mismatch aligned with established recoding-burden covariates in the topology-fixed Napolitano arginine setting; and realized design deviations from the Syn57 design were dominated by mutational accessibility rather than systematic optimization of mismatch cost. We frame these as a working three-layer hypothesis (§3.9.2) suitable for prospective follow-up rather than as a confirmed model. Natural code evolution itself appears constrained along two partly independent axes---physicochemical smoothness and codon-family topological integrity---both of which sit above the layer at which acute synthetic-recoding burden manifests.
