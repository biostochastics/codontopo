...
#figure(
  ...
  caption: [
    Summary of tRNA enrichment tests across 24 pairings (17 tRNAscan-SE--verified genomes, 5 variant genetic codes). MIS = maximal independent set, enumerated via Bron--Kerbosch algorithm on the conflict graph. Both MIS are significant at $p < 0.05$, eliminating the concern that greedy selection biased the independent-pairings result. The MIS worst-case ($p = 0.045$) serves as a robustness bound demonstrating that the result is not driven by a cherry-picked independent subset.
  ],
) <tbl:trna-tests>
