...
// Figures: tighter gap. Auto-placement allows figure floats so the
// surrounding text isn't broken up; multi-panel figures may take their
// own page, which is the intended layout for the 5-figure manuscript.
...
#block(
  ...
  The standard genetic code is structured to reduce the impact of point mutations, but the robustness of this property across physicochemical metrics, naturally variant codes, and codon-reassignment mechanisms remains incompletely quantified. We embed the 64 codons in $"GF"(2)^6$, representing the hypercube $Q_6$ as a coordinate-dependent subgraph of the encoding-independent single-nucleotide mutation graph $H(3,4)$. This framework allows interpolation between $Q_6$ and the full mutation graph and permits simultaneous analysis of physicochemical error minimization and codon-family topology. Under a block-preserving null model ($n = $10,000), the standard code is significantly low-cost across four amino-acid distance metrics: Grantham, Miyata, Woese polar requirement, and Kyte--Doolittle hydropathy ($p lt.eq 0.006$ for all metrics). The error-minimization signal strengthens monotonically as diagonal-edge weight increases from $Q_6$ toward $H(3,4)$. Across NCBI variant translation tables, this near-optimality is broadly preserved: among the 12 codes differing from the standard code by at least three reassignments, 11 retain top-5% placement after BH--FDR correction, with yeast mitochondrial code as the sole marginal exception; near-standard codes are also significant but are interpreted primarily as confirmations of standard-code geometry. Natural codon reassignments rarely break codon-family connectivity. Under $H(3,4)$, only #tk43.observed_breaks of #tk43.observed_total observed events increase amino-acid codon-family disconnectedness, compared with #str(calc.round(tk43.rate_possible * 100, digits: 0))% of 1,280 candidate single-codon relabelings (RR #str(calc.round(tk43.risk_ratio, digits: 2)), permutation $p lt.eq 10^(-4)$). The cross-lineage signal rests on the absence of breakers across most clades---22 of 28 events do not break topology, distributed across many lineages---rather than on multiple independent breakers being penalized: 4 of the 6 breakers come from a single mitochondrial lineage (yeast, NCBI table 3), and the breaker subset is genuinely underpowered for cross-clade structure. The depletion is nonetheless robust to alternative topology definitions, phylogenetic clade exclusions, and base-to-bit encodings, motivating $H(3,4)$ as the primary topology graph. Conditional-logit models show that topology avoidance and local physicochemical cost provide complementary explanatory signal, whereas a heuristic tRNA-distance proxy does not improve fit. Several variant-code lineages show suggestive enrichment of tRNA genes for reassigned amino acids, although topology-breaking subsets remain underpowered. Retrospective reanalysis of nine published genome-recoding datasets is consistent with---but does not establish---a working hypothesis in which codon-family topology, local mismatch geometry, and mutational accessibility operate at distinct biological layers; we frame this as exploratory and hypothesis-generating (§3.9.2). Together, the evolutionary findings support a two-axis model of genetic-code evolution in which physicochemical smoothness and codon-family topological integrity impose partly independent constraints.
...
To delimit the framework's algebraic scope, we examine four conjectures that were advanced by P.C. in a prior unpublished technical note #cite(<clayworth2026>) and circulated in informal methodological notes and presentations associated with this project---a Serine distance-4 invariance claim, PSL(2,7) symmetry, a holomorphic embedding extending a character of $"GF"(8)^*$, and a KRAS--Fano clinical prediction---and we test them here against the relevant data and report their falsification or rejection regardless of direction (@sec:res-negatives and Supplementary Note S1). The relevant claim statements are quoted verbatim where they are tested. Including these tests is intended to delimit scope, not to establish straw targets: each conjecture had a coherent algebraic motivation, and showing where the framework does *not* extend is part of giving an honest account of where it does.
...
We encode each nucleotide base as a 2-bit vector via the default bijection $phi: C |-> (0,0)$, $U |-> (0,1)$, $A |-> (1,0)$, $G |-> (1,1)$. A codon $b_1 b_2 b_3$ is then represented as the concatenation $phi(b_1) || phi(b_2) || phi(b_3) in "GF"(2)^6$. The 64 codons become the 64 vertices of the 6-dimensional hypercube $Q_6$. This default bijection was adopted in companion methodological work (#cite(<clayworth2026>, form: "prose")) prior to the encoding-sensitivity analysis reported here, on the grounds of visualization clarity: it places the standard code's nine 2-fold-degenerate amino acids on bit-5 differences (the natural geometry of wobble degeneracy). We do not claim this encoding is biologically privileged. All encoding-dependent results are reported under all 24 base-to-bit bijections (Section 2.1, end), and the load-bearing topology-avoidance result uses the encoding-independent $H(3,4)$ adjacency (Section 2.3.4).
...
Following the phylogenetic-distribution analysis of mitochondrial reassignment events by #cite(<sengupta2007>, form: "prose"), we performed clade-exclusion sensitivity analysis, iteratively removing each major taxonomic group (all ciliates, all metazoan mitochondria, all CUG-clade yeasts, etc.) and retesting. The depletion is highly significant in every regime ($p < 10^(-5)$). However, the structure of this finding warrants honest reading: the topology-breaking events themselves are not pan-taxonomic. Of the 6 topology-breaking reassignments under $H(3,4)$, 4 are concentrated in yeast mitochondrial (table 3); table 3 contributes 4 de-duplicated reassignments in total, all of them topology-breakers, so excluding that single clade leaves $28 - 4 = 24$ events with $6 - 4 = 2$ breakers, i.e., $2/24 = 8.3$% (hypergeometric $p$ moves from #sci(tq6.hypergeom_p) to $3.6 times 10^(-11)$). The clade-exclusion robustness is therefore a denominator effect---removing clades that contributed zero or one topology-breaking event each leaves the breakage rate essentially unchanged---rather than a numerator effect that would demonstrate breaking-then-failing across multiple lineages. The honest reading is: the *avoidance* of topology-breaking moves is pan-taxonomic, supported by 22 of 28 events that did not break topology distributed across many clades; the *attempts* to break topology are concentrated in one mitochondrial code, and with only 6 breakers the cross-clade structure of the breaker set is genuinely underpowered.

This concentration is itself informative, although the three diagnostics are not independent and we therefore avoid prior-probability calculations that would treat them as such. Yeast mitochondrial is the high-$k$, high-distortion outlier of the 27-table set ($k = 6$ reassignments; the most extensively reassigned code in the registry), so its surfacing as the marginal case in any analysis whose statistic scales with the magnitude of reassignment---the per-table optimality analysis (§3.3), the conditional-logit case with the lowest observed-move percentile (§3.5), and the topology-breaker concentration here---is a structural prediction of high-$k$ tables dominating $k$-aggregating diagnostics rather than three separate coincidences. The convergence is consistent with yeast mitochondrial occupying a distinct position in the codon-reassignment landscape---it is also the only variant code requiring acquisition of a novel tRNA derived from a different parent (tRNA#super[Thr] from tRNA#super[His]; @su2011)---but we do not interpret it as multi-test confirmation of any specific selectionist mechanism.
...
This subsection is exploratory and hypothesis-generating: we propose, and informally support, a three-layer view of how codon-space structure relates to recoding outcomes, but the partition into "evolutionary," "recoding-burden," and "design-deviation" layers was *not* prespecified before the analyses below. We therefore advance the layering as a working synthesis, not as a tested model. A confirmatory test of any one layer would require an independent dataset built around a sharper hypothesis than any single retrospective reanalysis can provide; we mark the layers as targets for future work where appropriate. The contrasts reported below remain individually informative, but their integration into a single three-layer schema is interpretive.
...
The second layer examines whether reassignment changes the physicochemical quality of the immediate mutational neighborhood. In the Syn57 contrast, serine swaps moved codons into better local Grantham neighborhoods ($Delta F_"local" = -37.2 plus.minus 30.5$), whereas alanine swaps moved codons into worse neighborhoods at a fixed $Delta F_"local" = +19.0 plus.minus 0.0$ (zero variance).

#block(fill: luma(245), inset: 8pt, radius: 3pt, width: 100%)[
  *Design-confounded contrast (descriptive, not inferential):* every Syn57 alanine swap goes to the same target codon, so all alanine $Delta_"local"$ values are identical. The Mann--Whitney $U = 0$, $p < 10^(-16)$ comparison reflects this design-level non-overlap rather than a within-group inferential test. We retain the numbers for transparency but they are *not* a hypothesis test of GF(2)#super[6] features and should not be cited as such.
]
 The methodologically clean inferential test comes from a topology-fixed setting---arginine synonymous recodings @napolitano2016, where all 12,888 AGR$arrow.r$CGN swaps remain within one connected component at $epsilon = 1$---in which local mismatch change correlated with established recoding-burden covariates: $Delta F_"local"$ vs RBS deviation ($rho = -0.33$, $p < 10^(-15)$) and vs mRNA folding deviation ($rho = -0.12$, $p < 10^(-15)$). Among the four CGN targets, CGG has the lowest local mismatch cost (246 Grantham units) while CGC has the highest (421), and target-specific RBS deviation spans four orders of magnitude, suggesting that local mismatch and SRZ covariates capture partially independent dimensions of recoding burden.
...
The synthetic recoding analyses, taken together, are consistent with (but do not prove) a three-layer view in which different structural descriptors operate at different biological layers: family topology for the geometry of reassignment space, local mismatch for some aspects of recoding burden, and Hamming accessibility for the short-range routes by which engineered designs deviate or revert. This partition is exploratory; each layer-claim is supported by a retrospective contrast rather than a prespecified test, and the integration into a single schema reflects post-hoc synthesis. We therefore label this as a working hypothesis suitable for prospective follow-up rather than as a confirmed model. Specific testable predictions: (i) a Syn-style recoding experiment that varies family-boundary crossing while holding local mismatch fixed should fail to predict transcriptomic burden (already partly observed in Syn57); (ii) a recoding scan that varies $Delta F_"local"$ within a single boundary regime should predict RBS-context proxies (consistent with the Napolitano arginine result); (iii) directed evolution starting from a recoded design should produce escape mutations whose Hamming distribution is shaped more by local accessibility than by mismatch optimisation (consistent with the Syn57 design-deviation analysis). Each prediction admits a direct experimental test that would either confirm or falsify the layer structure proposed here.
...
A natural question is whether the $"GF"(2)^6$ framework adds genuine insight beyond what a direct analysis of $H(3,4)$ would provide. The honest answer is: in two specific ways yes, in one circular way no. We list all three explicitly so that readers can weigh them independently of the rhetorical framing.
...
*(iii) Self-correcting machinery: the encoding-sensitivity audit.* The 24-encoding sweep distinguishes encoding-invariant properties (Serine disconnection, $H(3,4)$ topology depletion) from encoding-dependent ones (distance-4 claim, $Q_6$ topology depletion). This is genuinely useful---it caught the $Q_6$ depletion as encoding-dependent (8 of 24 bijections give no signal; @sec:res-topo, Supplement §S4) and prompted promotion of $H(3,4)$ to primary---but we acknowledge it is partly *self-correcting machinery for an artifact the binary framework introduced.* Pure $H(3,4)$ has no encoding choice and therefore needs no such audit.

The fair summary is that the binary representation is best understood as an analytical decomposition tool whose distinct contributions are the $rho$-sweep continuum and the $epsilon$-filtration; the encoding-sensitivity audit is methodological hygiene we owe the framework rather than a contribution from it. We accordingly present the encoding-independent $H(3,4)$ result as the primary topology-avoidance test and treat $Q_6$ as a coordinate-dependent decomposition useful for the $rho$-sweep continuum but not as a freestanding biological claim. We also emphasize that the Hamming-1/diagonal decomposition does not correspond to the biological transition/transversion partition: 16 of 24 encodings mix transitions and transversions equally among Hamming-1 edges, while 8 encodings place both transitions on diagonal edges (Supplement §S2). The $rho$ parameter is best interpreted as a diagonal-edge weight, not a transition/transversion weight.
...
Several limitations should be noted. First, the choice of base-to-bit encoding is not unique, and while the coloring optimality result holds across all 24 encodings, the specific score values and rank orderings are encoding-dependent (full encoding-sensitivity results across all 24 bijections are provided in the Supplementary Material). Second, the block-preserving null model, while standard in the field @freeland1998, preserves more structure than a fully random code and may understate the degree of optimality. Third, the tRNA enrichment result, while robust to pairing selection (worst-case MIS $p = 0.045$), relies on a small number of independent pairings ($n = 6$) with limited statistical power; this result is appropriately classified as suggestive and should not be interpreted as demonstrating a universal compensation mechanism. The MAC-genome assemblies for the more deeply sampled ciliates vary substantially in fragmentation (visible in the predicted-pseudogene fraction of Supplement §S10: 79/495 = 16% in _E. weissei_, 18/262 = 7% in _P. persalinus_, 0--5% elsewhere). Removing the two genomes with $> 10$% pseudogene fraction (_E. weissei_, _P. persalinus_) yields 22 of the original 24 pairings; the all-pairings Stouffer combination on this restricted set gives $p = 0.041$ (we report the Stouffer all-pairings $p$ here rather than re-running Bron--Kerbosch on the reduced conflict graph, since with two organisms removed the MIS structure shifts non-trivially and the comparison to the headline 24-pairing all-pairings $Z = 5.10$ is the more interpretable stress test). The suggestive result is therefore not driven by the most fragmented assemblies; we note this only as a sensitivity check and not as an additional inferential test. We also flag a small consistency caveat: the _Saccharomyces cerevisiae_ tRNA counts used in the yeast-mitochondrial Thr disconnection pairing come from GtRNAdb @gtrnadb rather than from a tRNAscan-SE 2.0.12 run on the assembly, in contrast to the tRNAscan-SE-verified genomes in Table 8 and Supplement §S10. We retain that case rather than re-run because the GtRNAdb counts are the standard-of-record for yeast and rerunning produces nearly identical numbers; the pairing is unaffected by the choice. Fourth, the conditional logit model (@sec:res-condlogit) is an event-level explanatory model, not direct proof of biological causality; the candidate universe comprises all $approx $1,280 single-codon reassignments regardless of mechanistic plausibility, and the tRNA-complexity proxy (Hamming distance to nearest target-AA codon) is heuristic rather than mechanistically grounded. The finding that this particular proxy did not improve model fit does not exclude the possibility that a richer model of tRNA repertoire change would contribute explanatory value.

Fifth, a survivorship-bias caveat applies to all event-level analyses: the de-duplicated reassignment events analyzed here are reassignments that _persisted_ in extant lineages. Topology-breaking reassignments that produced fitness collapse and lineage extinction are unobservable. The depletion result is therefore consistent with both selection against attempting topology-breaking moves and selection against the lineages that attempted them; cross-sectional NCBI data cannot adjudicate between these. Both interpretations support the conclusion that codon-family connectivity constrains evolutionary trajectories, but with different mechanistic implications for the locus of selection. Sixth, multiple-comparison correction was applied within analysis families (metric, $rho$-sweep, per-table, topology-avoidance, synthetic-recoding) rather than across all descriptive, exploratory, and confirmatory quantities. We did not file an external pre-registration; the family structure is the natural one for these analyses but should be read as a working-document organisation rather than as a formally pre-registered design. The cross-family logical-independence claim is doing real work and we defend it explicitly: the eight test families address conceptually distinct questions (cross-metric robustness of the standard code; $rho$-interpolation between $Q_6$ and $H(3,4)$; per-NCBI-table preservation under each table's own block-preserving null; the $2 times 2$ definition $times$ adjacency audit of topology avoidance; clade-exclusion sensitivity matching #cite(<sengupta2007>, form: "prose"); the M1--M4#sub[H(3,4)] discrete-choice model comparison; the 24-pairing tRNA-enrichment Stouffer combination with MIS robustness; and the cross-study recoding reanalysis), and their test statistics are non-nested (a hypergeometric depletion ratio cannot be reframed as a likelihood-ratio for a discrete-choice model, etc.). For transparency we list which findings would survive a Bonferroni correction across all eight families if such an aggregation were imposed: the headline H(3,4) topology depletion at hypergeometric $p = 1.3 times 10^(-6)$ would (cross-family $alpha = 0.05/8 = 6.25 times 10^(-3)$); the cross-metric coloring optimality at minimum $p = 0.006$ across four metrics would (already at the threshold); the $rho$-sweep at $p lt.eq 0.006$ across all five $rho$ would; the conditional-logit ΔAICc gap of #str(calc.round(m1f.aicc - m3f.aicc, digits: 0)) (M1$arrow.r$M3) is well above any threshold; the per-table BH--FDR result is robust by construction. The MIS worst-case tRNA enrichment at $p = 0.045$ would *not* survive cross-family Bonferroni and we are explicit that this is a suggestive, underpowered result rather than confirmatory evidence. Within families, the primary claims remain significant under appropriate correction: all four physicochemical metrics pass Bonferroni correction across the four-metric family ($alpha = 0.05/4 = 0.0125$); all five $rho$ values pass Bonferroni correction across the $rho$-sweep family ($alpha = 0.05/5 = 0.01$); per-table tests are reported with BH--FDR (#pt.n_significant_bh of #pt.n_tables tables significant after correction); and topology avoidance remains significant at $p < 10^(-6)$ under both adjacency definitions and both topology-breaking definitions after Bonferroni correction across the $2 times 2$ definition $times$ adjacency family ($alpha = 0.05/4 = 0.0125$). The three Ostrov segment tests use Bonferroni correction within their own family ($alpha = 0.05/3 = 0.0167$). Exploratory and suggestive analyses are labeled as such and are not interpreted as confirmatory without the caveats stated in their respective sections.
...
The standard genetic code is robustly low-cost: across four amino acid distance metrics and across a continuum of mutation graphs spanning $Q_6$ and the complete single-nucleotide graph $H(3,4)$, it consistently sits in the extreme tail of block-preserving null models ($p lt.eq 0.006$ under every combination). Across the 27 NCBI translation tables this near-optimality is preserved at the variant level: of the #pt.informative_total codes whose distance from the standard makes the per-table test informative, #pt.informative_significant retain top-5% placement after BH--FDR correction, with only yeast mitochondrial (table 3) marginally above the 5% threshold; the #pt.near_standard_total near-standard tables are formally significant in the same correction but read primarily as confirmatory of standard-code geometry. Code evolution therefore appears to occur within a constrained low-cost region rather than freely across code space.

Two complementary constraints govern that constrained region. First, persistent natural reassignments preferentially enter neighborhoods of lower local Grantham mismatch. Second, they avoid fragmenting amino-acid codon-family connectivity: topology-breaking moves are approximately #str(calc.round(tk43.depletion_fold, digits: 1))-fold depleted under the encoding-independent $H(3,4)$ adjacency, and a conditional-logit decomposition shows that topology adds explanatory value beyond physicochemistry, and vice versa, by margins well above the conventional ΔAICc>10 reference ($Delta"AICc" gt.eq #str(calc.round(calc.min(cl.model_fits.M1_phys.aicc - cl.model_fits.M3_phys_topo.aicc, cl.model_fits.M2_topo.aicc - cl.model_fits.M3_phys_topo.aicc), digits: 0))$ under $Q_6$ topology; $Delta"AICc" gt.eq #str(calc.round(calc.min(cl.at("encoding_robustness", default: (:)).at("delta_aicc_M1_to_M3_k43", default: 0), cl.at("encoding_robustness", default: (:)).at("delta_aicc_M2k43_to_M3k43", default: 0)), digits: 0))$ under encoding-independent $H(3,4)$ topology). Because $Q_6$ adjacency is encoding-dependent (8 of 24 base-to-bit bijections give no $Q_6$ topology depletion in the candidate landscape), the survival of M3 dominance under $H(3,4)$ rules out a coordinate artifact: the topology constraint is a property of nucleotide-level codon-family connectivity, not of the binary representation. The two axes are only weakly correlated ($r_s = #str(calc.round(cl.phys_topo_rho, digits: 2))$), the topology coefficient is robust to all seven phylogenetic clade exclusions ($Delta"AICc"("M1" arrow.r "M3") gt.eq 49$ in the worst regime), and a posterior-predictive simulation under M3 reproduces the observed topology-breaking rate ($p = 0.60$). We make no causal claim about the mechanism---tRNA cross-recognition range, ribosomal decoding geometry, and other facets of translational architecture remain plausible explanations not separately modeled here---and the cross-sectional NCBI data cannot distinguish selection against attempting topology-breaking moves from selection against lineages that attempted them. Several variant-code lineages do show suggestive tRNA-gene enrichment for the reassigned amino acid (worst-case MIS Stouffer $p = #str(calc.round(trna.mis_worst_p, digits: 3))$ across 24 pairings from 18 tRNAscan-SE-verified genomes), consistent with compensatory accommodation; the topology-breaking-restricted subset alone is underpowered ($n = 4$).

A retrospective cross-study reanalysis of eight published genome-recoding experiments ($>$217,000 codon-level observations) is consistent with---but does not establish---a working hypothesis in which the same codon-space descriptors operate at different biological layers in evolutionary versus engineering contexts. Family-boundary crossing is a real structural distinction over evolutionary timescales but did not predict acute transcriptomic perturbation in the Syn57 contrast; local neighborhood mismatch aligned with established recoding-burden covariates in the topology-fixed Napolitano arginine setting; and realized design deviations from the Syn57 design were dominated by mutational accessibility rather than systematic optimization of mismatch cost. The three-layer integration is exploratory: each layer-claim rests on a single retrospective contrast, and the partition into "evolutionary," "recoding-burden," and "design-deviation" axes was not pre-specified (§3.9.2). We therefore frame this as a hypothesis suitable for prospective follow-up rather than a confirmed model. Natural code evolution itself, by contrast, appears constrained along two partly independent axes---physicochemical smoothness and codon-family topological integrity---both of which sit above the layer at which acute synthetic-recoding burden manifests.

The framework's algebraic ambitions remain bounded. The KRAS--Fano clinical prediction, Serine distance-4 invariant, PSL(2,7) symmetry, and holomorphic-embedding claims were tested and rejected (@tbl:claims) and are out of scope. The $"GF"(2)^6$ representation contributes two genuine pieces of analytical machinery---the $rho$-sweep that interpolates between $Q_6$ and $H(3,4)$, and the $epsilon$-filtration that gives codon-family connectivity a persistence parameter---plus a third, the encoding-sensitivity audit, that is best understood as methodological hygiene the binary representation owes the framework rather than as a contribution from it. What survives is a measured, encoding-independent picture of the genetic code as a low-cost coloring of $H(3,4)$ whose persistent reassignment trajectories respect both the physicochemistry of the substituted amino acid and the connectivity of the codon family it joins.
...
All code, raw data, and intermediate analysis outputs are publicly released in the `codontopo` repository at https://github.com/biostochastics/codontopo (version #stats._version, commit #raw(stats.at("_commit", default: "see repo HEAD"))). Install with `pip install -e ".[all]"` or `uv sync --all-extras`. Analyses are fully reproducible via:
