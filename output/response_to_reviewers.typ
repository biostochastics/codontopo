...
#let horizontalrule = line(length: 100%, stroke: 0.4pt + gray)
= Response to Reviewers --- BIOSYS-D-26-00689
<response-to-reviewers--biosys-d-26-00689>
#strong[Manuscript:] Robust error-minimization in the genetic code across physicochemical metrics and variant codes: a graph-theoretic analysis in GF(2)⁶
#strong[Authors:] Paul Clayworth, Sergey Kornilov
...
<reviewer-1>
#quote(block: true)[
#emph[\"The manuscript would also benefit from discussion of recent work by Ekaterina Yurova Axelsson and Andrei Khrennikov on 2-adic and dynamical-system representations of the genetic code published in BioSystems …\"]
]
...
The bibliography now includes Yurova Axelsson & Khrennikov 2024a (#emph[BioSystems] 240:105230), 2024b (#emph[BioSystems] 246:105353), Khrennikov & Kozyrev (2007, #emph[Physica A] 381:265), and Dragovich & Mišić (2019, #emph[BioSystems] 185:104017).

We have added a second Walsh invariant of the same wobble structure (Supplement §S16.3): the wobble-free label-spectrum fraction #emph[S] = 0.7514309076, also encoding-invariant across all 24 base-to-bit bijections. The standard code\'s amino-acid labeling concentrates Walsh-Hadamard energy on the wobble-free spectral layer to a degree unattainable under a degeneracy-multiset-preserving label permutation (#emph[z] = +30.3, #emph[p] \< 10⁻⁴ at #emph[n] = 10,000), but is mathematically invariant under the stricter wobble-box-preserving label permutation --- so #emph[S] is a spectral signature of biological wobble-box alignment, not evidence of additional optimisation. Together with spectral\_depth = 544, the two encoding-invariant Walsh signatures provide a complete algebraic characterisation of the wobble structure in the discrete shadow of Khrennikov\'s ultrametric framework. We explicitly disclaim a \"beyond wobble\" interpretation in §S16.3 and the Discussion.
...
<r21--overlap-among-the-four-physicochemical-measures>
#quote(block: true)[
#emph[\"The list of models taken for the analysis by Clayworth and Kornilov is thus inconsistent; the experimentally obtained polarity variable by Woese overlaps with its statistical correction by Grantham, and 3 variable model by Grantham overlaps with the 2 variable version by Miyata et al. (1979).\"]
]
...
- The Supplement claim-hierarchy entry for #emph[hypercube\_coloring\_optimality] (§S1) has been updated in the same way.

To quantify the degree of overlap, we added metric-correlation analyses at two levels (§3.1; Supplement §S15):

- #emph[Pairwise Spearman across the 190 unordered amino-acid pairs:] range 0.23--0.73; median 0.49. Highest pairs (Grantham--Miyata ρ = 0.71, Miyata--PR ρ = 0.73) confirm the reviewer\'s point about polarity-component overlap.
- #emph[Pairwise Spearman across 2,000 random codes drawn from the same Freeland--Hurst null:] range 0.71--0.89; median 0.82. Code-level rankings are more correlated than AA-pair distances, as expected when distances are summed over 192 hypercube edges.
- #emph[Partial Spearman correlations of code-level F-scores, controlling for the other measures:] five of ten pairs become statistically independent (|partial ρ| \< 0.1) after partialling. Substantial shared variance coexists with non-identical residual structure across the panel.

We now describe the cross-metric concordance as a #emph[convergent sensitivity envelope] rather than independent replication. The claim is that if the standard code lies in the low-cost tail under several established but non-identical physicochemical parameterizations, the result is not confined to a single chosen scale. With five tests at α = 0.05, the Bonferroni-corrected per-test threshold is 0.01; all five p-values pass (Grantham p = 0.006; Miyata p \< 0.001; Woese polar requirement p = 0.003; Kyte--Doolittle p = 0.001; ProtSub p = 0.0004).

=== R2.2 --- ProtSub matrix
<r22--protsub-matrix>
#quote(block: true)[
#emph[\"It would be better if the authors tested the method on, e.g. ProtSub matrix - derived \'to bring sequence alignments into agreement with structure matches\' (Jia K, Jernigan RL. Proteins. 2021; 89:671-682. doi.org/10.1002/prot.26050.).\"]
]
...
ProtSub is provided by Jia & Jernigan as a log-odds substitution matrix in EMBOSS format. We converted it to a positive distance using the standard diagonal-anchored relation #emph[d(i,j) = ½(s(i,i) + s(j,j)) − s(i,j)];, which guarantees #emph[d(i,i) = 0] and is strictly positive off-diagonal for ProtSub (verified). Sanity values are physicochemically sensible: d(I,L) = 2.0, d(K,R) = 4.0, d(F,Y) = 4.0, d(D,E) = 3.5, d(C,W) = 14.5.
...
<slavov-et-al-2026--convergent-event-level-context>
We have also added a Discussion paragraph (§4.6) noting the very recent publication of Tsour, Machné, Leduc, Widmer, Koo, Guez, Karczewski & Slavov (2026, #emph[Nature];, doi:10.1038/s41586-026-10678-2, \"Alternate RNA decoding results in stable and abundant proteins in mammals\"). This work directly observes 8,746 unique amino-acid substitutions across 1,767 genes in human and mouse tissues arising from sense-codon recoding, with codon--anticodon mismatches identified as a primary mechanism.

For each high-confidence event in their Supplementary Data 8 (5,873 events; 5,611 unambiguous-pair events; positional probability \> 0.9) we computed the minimum nucleotide-Hamming distance between any source codon and any target codon under the standard genetic code (Supplement §S17). At the #emph[event] level, 65.0 % of observations involve source/target codon pairs at min-NT-1, versus a 39.5 % baseline among all 380 ordered amino-acid pairs (binomial test, one-sided p \< 10⁻¹⁰⁰). At the level of unique substitution #emph[types];, the enrichment is modest (41.6 % observed vs 39.5 % baseline; Fisher exact p = 0.26; Mann--Whitney U on NT distances, observed vs unobserved types, p = 0.18); almost every amino-acid substitution type can be observed in their dataset, but the #emph[frequency] of each type is biased toward single-nucleotide-distance codon pairs. We describe this in the Discussion as convergent event-level context for the biological relevance of local codon neighbourhoods, with the unique-pair analyses reported as more conservative.
...
+ Pairwise Spearman correlations of the four code-independent measures plus ProtSub across the 190 unordered AA pairs (Supplement §S15).
+ Code-level F-score correlations and partial F-score correlations among the five measures across 2,000 random codes (Supplement §S15).
+ ProtSub as a fifth measure, with full Freeland--Hurst null Monte Carlo (n = 10,000).
+ Walsh--Hadamard / 2-adic spectral probe with two null tests, block-size matched and wobble-box-preserving label permutation (Supplement §S16).
+ Wobble-free label-spectrum invariant S = 0.7514 (Supplement §S16.4).
+ Slavov SAAP cross-analysis (Supplement §S17).

#strong[New code (all in repository):]
...
#strong[New references added to bibliography:]

- Yurova Axelsson & Khrennikov 2024a (#emph[BioSystems] 240:105230)
- Yurova Axelsson & Khrennikov 2024b (#emph[BioSystems] 246:105353)
- Khrennikov & Kozyrev 2007 (#emph[Physica A] 381:265)
- Dragovich & Mišić 2019 (#emph[BioSystems] 185:104017)
- Di Giulio 2001 (#emph[J Theor Biol] 208:141)
- Jia & Jernigan 2021 (#emph[Proteins] 89:671)
- Buschmann et al. 2026 (#emph[Bioinformatics] 42:btag188)
- Kosiol et al. 2004 (#emph[J Theor Biol] 228:97)
- Tsour et al. 2026 (#emph[Nature];, doi:10.1038/s41586-026-10678-2)

#strong[Manuscript edits:]
