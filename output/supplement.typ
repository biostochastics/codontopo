...
*Independence-graph caveat.* The conflict graph encodes shared *organisms*, not phylogenetic distance. Two ciliates from different orders are treated as independent in this construction even though they may share more evolutionary signal than two arbitrary eukaryotes; conversely, both MIS retain the _S. cerevisiae_ mito/Thr pairing whose tRNA counts come from GtRNAdb rather than from a tRNAscan-SE 2.0.12 run on the assembly (see §S10.2). A phylogenetic-distance--based independence criterion would yield a smaller effective $n$ (the same 4--6 phylogenetically independent origins discussed in main-text Limitations); the MIS analysis should be read as bounding the worst case under the organism-shared-by-pairings criterion, not as a phylogeny-aware test.
...
All analyses were performed using the `codon-topo` Python package (version #stats._version, commit #raw("2f1ba6a"), tag #raw("v0.4.0")). The code is publicly released at https://github.com/biostochastics/codontopo. Dependencies and runtime requirements:
...
The `[all]` extra installs every dependency required to reproduce the analyses; the `[dev]` extra additionally installs the test toolchain. The complete test suite (432 tests) is then runnable with `python3.11 -m pytest tests/ -m "not slow"` after `pip install -e ".[dev]"`.
