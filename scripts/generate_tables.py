...
mc = monte_carlo_null(n_samples=10_000, seed=135325, include_null_samples=True)
...
# CLAUDE.md convention: coloring-optimality MC uses n=10,000 across all panels.
print("Table 4: Per-table optimality (n=10000)")
from codon_topo.analysis.coloring_optimality import per_table_optimality

pt = per_table_optimality(n_samples=10_000, seed=135325)
rows = []
...
# CLAUDE.md convention: coloring-optimality MC uses n=10,000 across all panels.
print("Table 5: Rho robustness sweep (n=10000)")
...
topo = topology_breaking_subset_test()
...
write_csv(
    ...
    [
        ...
        {
            "test": "MIS enumeration (median)",
            "n_pairings": mis["n_pairings"],
            "statistic": f"{mis['n_mis_total']} MIS of size 6",
            "p_value": round(mis["median_stouffer_p"], 6),
        },
        {
            "test": "MIS enumeration (worst-case)",
            "n_pairings": mis["n_pairings"],
            "statistic": f"{int(mis['fraction_significant_p05'] * mis['n_mis_total'])}/{mis['n_mis_total']} MIS at p<0.05",
            "p_value": round(mis["worst_case_stouffer_p"], 6),
        },
        {
            "test": "Topology-breaking subset",
            "n_pairings": topo["n_pairings"],
            "statistic": f"Z={topo['stouffer_z_topology_breaking']:.3f}",
            "p_value": round(topo["stouffer_p_topology_breaking"], 6),
        },
    ],
)
...
# Emits the paper's PRIMARY cell of the 2x2 audit: H(3,4) adjacency
# (encoding-independent) with the Δβ₀>0 topology-breaking definition
# (Methods §2.3.4). The four-cell audit lives in tbl:s-topo-audit; FigF
# (supplement §S3) reads this CSV expecting the primary cell.
print("Table 9: Topology avoidance (primary: H(3,4), delta_beta_0 > 0)")
from codon_topo.analysis.synbio_feasibility import (
    topology_avoidance_k43,
    topology_definitions_audit,
)

# Primary cell (H(3,4), Δβ₀>0) numbers come from the 2x2 audit;
# permutation p and n_permutations are reported by topology_avoidance_k43,
# the encoding-independent H(3,4) test that the manuscript declares
# primary (Methods §2.3.4).
audit = topology_definitions_audit()
primary = next(r for r in audit["audit_rows"] if r["cell"] == "k43_beta0")
k43_ta = topology_avoidance_k43()
write_csv(
    ...
    [
        {
            "cell": primary["cell"],
            "adjacency": "H(3,4)",
            "definition": "delta_beta_0 > 0",
            "observed_creates_disc": primary["observed_breaks"],
            "observed_total": primary["observed_total"],
            "rate_observed_pct": round(100 * primary["rate_observed"], 1),
            "possible_creates_disc": primary["possible_breaks"],
            "possible_total": primary["possible_total"],
            "rate_possible_pct": round(100 * primary["rate_possible"], 1),
            "hypergeom_p": primary["hypergeom_p"],
            "permutation_p": k43_ta.get("permutation_p"),
            "n_permutations": k43_ta.get("n_permutations"),
            "risk_ratio": primary["risk_ratio"],
            "depletion_fold": primary["depletion_fold"],
        }
    ],
)
