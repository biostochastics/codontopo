...
_PROTSUB_CACHE: dict | None = None

# Type alias for a distance function: (aa1_1letter, aa2_1letter) -> float
DistanceFunc = Callable[[str, str], float]

# Available metric names for external use. "protsub" is a code-dependent
# (MSA-derived) robustness metric; it falls under the Di Giulio (2001)
# tautology caution and is reported as a structure-aware supplementary
# check, not a primary physicochemical scale.
AVAILABLE_METRICS = (
    "grantham",
    "miyata",
    "polar_requirement",
...
# Subset that is code-independent (true physicochemical metrics).
CODE_INDEPENDENT_METRICS = (
    "grantham",
    "miyata",
    "polar_requirement",
    "kyte_doolittle",
...
def _load_protsub() -> dict:
    """Load the ProtSub-derived distance matrix from data/protsub.json.

    ProtSub (Jia & Jernigan 2021) is a structure-aware substitution matrix
    derived from MSAs of 2,320 Pfam domains. The stored matrix is the
    normalized-log-odds distance d(i,j) = (s(i,i) + s(j,j))/2 - s(i,j)
    in the range ~0 to 15. Diagonal is 0; off-diagonal strictly positive.

    Caveat: ProtSub is derived from sequences encoded by the standard
    genetic code, so it is *code-dependent* in the sense of Di Giulio
    (2001). It is included as a robustness check against a code-dependent
    metric, not as a primary physicochemical scale.
    """
    global _PROTSUB_CACHE
    if _PROTSUB_CACHE is not None:
        return _PROTSUB_CACHE
    _PROTSUB_CACHE = _load_matrix_json("protsub.json")
    return _PROTSUB_CACHE
...
# ProtSub max off-diagonal distance under the diagonal-anchored conversion.
_PROTSUB_MAX = 15.0


def protsub_distance(aa1: str, aa2: str) -> float:
    """Return diagonal-anchored ProtSub distance between two amino acids.

    Derived from Jia & Jernigan (2021) by d(i,j) = (s(i,i) + s(j,j))/2 - s(i,j).
    Range ~0 to 15. See `_load_protsub` for caveats.
    """
    if aa1 == "Stop" or aa2 == "Stop":
        return (STOP_ADJACENCY_PENALTY * _PROTSUB_MAX / 215.0) if aa1 != aa2 else 0.0

    matrix = _load_protsub()
    a1 = _canonicalize_aa(aa1)
    a2 = _canonicalize_aa(aa2)
    return float(matrix[a1][a2])
...
def rho_robustness_sweep(
    ...
    # Per CLAUDE.md, the coloring-optimality Monte Carlo (headline result)
    # uses n = 10,000. Keep this in sync with monte_carlo_null and
    # per_table_optimality so downstream figures/tables agree on sample size.
...
def _generate_random_code_haig_hurst_aa_permutation(
    reference_code: dict[str, str],
    rng: random.Random,
) -> dict[str, str]:
    """Classical Haig--Hurst 1991 / Freeland--Hurst 1998 AA-permutation null.

    Preserves BOTH the position of stop codons AND the codon-family
    membership (which codons belong to which family). Randomises: assigns
    a uniformly random permutation of the 20 amino-acid labels to the 20
    sense-codon families. Total possible codes: 20! (approx 2.43e18).

    This is a WEAKER null than the quartet-pattern shuffle: it does not
    preserve the codon-family-to-quartet-slot assignment, so a random code
    under this null can place amino acid X's family anywhere in the codon
    space, not only at the quartet slot where X's family sits in the
    reference code.

    Historical reference: Haig, D. & Hurst, L.D. 1991, J Mol Evol
    33:412-417; Freeland, S.J. & Hurst, L.D. 1998, J Mol Evol 47:238-248.
    """
    families: dict[str, list[str]] = defaultdict(list)
    for codon, aa in reference_code.items():
        families[aa].append(codon)

    sense_aas = sorted(aa for aa in families if aa != "Stop")
    stop_codons = families.get("Stop", [])

    permuted_aas = list(sense_aas)
    rng.shuffle(permuted_aas)

    perm_code: dict[str, str] = {c: "Stop" for c in stop_codons}
    for old_aa, new_aa in zip(sense_aas, permuted_aas):
        for codon in families[old_aa]:
            perm_code[codon] = new_aa
    return perm_code
...
def monte_carlo_null(
    ...
    include_stops: bool = True,
    metric: str = "grantham",
    include_null_samples: bool = False,
) -> dict:
    ...
    If ``include_null_samples`` is True, the full list of n_samples null
    scores is included in the returned dict under key ``null_samples`` so
    downstream code (e.g. figures) can plot the empirical histogram rather
    than a Gaussian approximation of it. Off by default because internal
    callers such as per_table_optimality, rho_robustness_sweep, and the
    24-encoding sensitivity sweep invoke this function many times over
    and would otherwise multiply the serialised JSON size by ~n_samples.
    """
    ref = reference_code if reference_code is not None else STANDARD
    rng = random.Random(seed)
    dist_fn = get_distance_func(metric)

    if null_type not in ("freeland_hurst", "class_size", "haig_hurst_aa"):
    ...
    for _ in range(n_samples):
        ...
        elif null_type == "haig_hurst_aa":
            random_code = _generate_random_code_haig_hurst_aa_permutation(ref, rng)
        else:
    ...
    if include_null_samples:
        # Full null draws (~200 KB at n=10,000) so figures can plot the
        # empirical histogram instead of a Gaussian approximation.
        result["null_samples"] = list(null_scores)
    return result
...
def per_table_optimality(
    # Per CLAUDE.md, the coloring-optimality Monte Carlo (headline result)
    # uses n = 10,000. The per-table variant is part of the same workstream
    # (Fig 2C, FigB, Methods §2) and must use the same n so p-value floors
    # (1/(n+1)) match across all reported panels.
    n_samples: int = 10_000,
    seed: int | None = None,
) -> dict:
...
def multi_metric_sensitivity(
    ...
    Defaults to the four code-independent physicochemical metrics so that
    the primary publishable result does not silently inherit code-dependent
    (MSA-derived) metrics like ProtSub, which fall under the Di Giulio
    (2001) tautology caution. ProtSub is available via explicit opt-in
    (`metrics=list(AVAILABLE_METRICS)` or by passing the full list).
    """
    if metrics is None:
        metrics = list(CODE_INDEPENDENT_METRICS)
...
# ====================================================================
# Mechanistic discriminant test: codon capture vs ambiguous intermediate
# ====================================================================
...
def mechanistic_discriminant_test() -> dict:
    ...
    Hypothesis: If topology-breaking changes involve smaller physicochemical
    jumps than topology-preserving ones, this supports "codon capture"
    (small jumps tolerated without fitness catastrophe). If not, it supports
    the "ambiguous intermediate" model (large jumps require topology
    preservation to maintain decoding).
    """
    from scipy.stats import mannwhitneyu
...
# ====================================================================
# Metric correlation analyses (Supplement §S15)
# ====================================================================


def aa_pair_metric_correlations() -> dict:
    """Pairwise Spearman correlations across the 190 unordered AA pairs
    for all metrics in AVAILABLE_METRICS.

    For each metric pair (M1, M2) we evaluate the distance under M1 and
    M2 on all (i, j) pairs of the 20 standard amino acids (i < j; 190
    pairs) and compute the Spearman rank correlation. Returns the 5x5
    matrix plus off-diagonal min/median/max summaries.
    """
    from itertools import combinations
    from scipy.stats import spearmanr

    AAS = list("ARNDCQEGHILKMFPSTWYV")
    pairs = list(combinations(AAS, 2))  # 190 ordered pairs (i < j)

    vectors: dict[str, list[float]] = {}
    for m in AVAILABLE_METRICS:
        fn = get_distance_func(m)
        vectors[m] = [fn(a, b) for (a, b) in pairs]

    rho: dict[str, dict[str, float]] = {m: {} for m in AVAILABLE_METRICS}
    for m1 in AVAILABLE_METRICS:
        for m2 in AVAILABLE_METRICS:
            if m1 == m2:
                rho[m1][m2] = 1.0
            else:
                r, _ = spearmanr(vectors[m1], vectors[m2])
                rho[m1][m2] = round(float(r), 4)  # type: ignore[arg-type]

    offdiag = [rho[a][b] for a, b in combinations(AVAILABLE_METRICS, 2)]
    sorted_off = sorted(offdiag)
    return {
        "metrics": list(AVAILABLE_METRICS),
        "n_pairs": len(pairs),
        "spearman_rho_matrix": rho,
        "offdiagonal_min": round(min(offdiag), 4),
        "offdiagonal_median": round(sorted_off[len(sorted_off) // 2], 4),
        "offdiagonal_max": round(max(offdiag), 4),
    }


def code_level_F_score_correlations(
    n_null: int = 2000,
    seed: int = 135325,
) -> dict:
    """Pairwise Spearman correlations of F-scores across n_null random
    codes drawn from the Freeland-Hurst null, for all metrics.

    Also computes partial Spearman correlations: for each pair (M1, M2),
    rank-residualise each F-vector against the rank-projection onto the
    other metrics and report the residual correlation. Five of ten pairs
    typically become independent (|partial rho| < 0.1) after partialling,
    indicating that each metric contributes unique variance.
    """
    ...
    dist_fns = {m: get_distance_func(m) for m in metrics}

    F_null: dict[str, list[float]] = {m: [] for m in metrics}
    for _ in range(n_null):
        code = _generate_random_code_freeland_hurst(STANDARD, rng)
        for m in metrics:
            F_null[m].append(
                hypercube_edge_mismatch_score(code, distance_func=dist_fns[m])
            )

    rho_F: dict[str, dict[str, float]] = {m: {} for m in metrics}
    for m1 in metrics:
        for m2 in metrics:
            if m1 == m2:
                rho_F[m1][m2] = 1.0
            else:
                r, _ = spearmanr(F_null[m1], F_null[m2])
                rho_F[m1][m2] = round(float(r), 4)  # type: ignore[arg-type]
    ...
    def _partial_spearman(
        ...
        rx = rankdata(x)
        ry = rankdata(y)
        rcs = [rankdata(c) for c in controls]
        Z = _np.column_stack(rcs)
        rx_resid = rx - Z @ _np.linalg.lstsq(Z, rx, rcond=None)[0]
        ry_resid = ry - Z @ _np.linalg.lstsq(Z, ry, rcond=None)[0]
        return float(_np.corrcoef(rx_resid, ry_resid)[0, 1])

    partials: dict[str, float] = {}
    for m1, m2 in _comb(metrics, 2):
        others = [m for m in metrics if m not in (m1, m2)]
        partials[f"{m1}_vs_{m2}"] = round(
            _partial_spearman(F_null[m1], F_null[m2], [F_null[k] for k in others]),
            4,
        )

    n_independent = sum(1 for v in partials.values() if abs(v) < 0.1)

    offdiag = [rho_F[a][b] for a, b in _comb(metrics, 2)]
    sorted_off = sorted(offdiag)
    return {
        "metrics": metrics,
        "n_null_codes": n_null,
        "F_spearman_rho_matrix": rho_F,
        "F_offdiagonal_min": round(min(offdiag), 4),
        "F_offdiagonal_median": round(sorted_off[len(sorted_off) // 2], 4),
        "F_offdiagonal_max": round(max(offdiag), 4),
        "partial_spearman_rho": partials,
        "n_partials_independent": n_independent,
        "n_partial_pairs_total": len(partials),
    }
