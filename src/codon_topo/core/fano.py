...
def _reverse_lookup(
    ...
    if key not in _REVERSE_CACHE:
        _REVERSE_CACHE[key] = {codon_to_vector(c, enc): c for c in ALL_CODONS}
...
def is_fano_line(
    ...
    enc = encoding or DEFAULT_ENCODING
    v1, v2, v3 = (
        codon_to_vector(c1, enc),
        codon_to_vector(c2, enc),
        codon_to_vector(c3, enc),
    )
...
def all_single_bit_fano_lines(
    ...
    for bit_pos in range(6):
        flipped = list(v)
        flipped[bit_pos] = 1 - flipped[bit_pos]
        mutant = tuple(flipped)
