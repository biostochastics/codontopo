...
# Round-2 audit fix (T17): read observed counts, χ², and p-value from JSON
# rather than hard-coding — the previous literals silently held stale values
# when the reassignment corpus changed. `reassignment_analysis.json` is the
# single source of truth used by manuscript_figures.R Fig3A as well.
...
chi2_stat <- bit_bias_json$chi2_statistic_uniform_reference
chi2_p    <- bit_bias_json$chi2_p_value_uniform_reference
...
p3 <- ggplot(bit_data, aes(x = label, y = count, fill = region)) +
  ...
  labs(
    ...
    subtitle = bquote(chi^2 ~ "=" ~ .(sprintf("%.2f", chi2_stat)) * "," ~
                      italic(p) ~ "=" ~ .(sprintf("%.3f", chi2_p)) ~ "(" *
                      italic(n) ~ "=" ~ .(total_n) ~
                      "bit-changes, 27 NCBI tables)"),
...
# would silently drift if calibration_points ever changed.
depth_json <- fromJSON(file.path(input_dir, "depth_calibration.json"))
spearman_rho <- depth_json$correlation$spearman_rho

# Build annotation labels from the CSV: one label per lineage-age cluster.
# Use the pre-LUCA Ser row, the ~150 Mya CUG-clade row (Ala, tab26), and the
# ~600 Mya Chlorophyceae row (Leu, tab16). Lineage name derives from the CSV
# so lineage-rename shows up automatically.
ser_row <- depth[depth$aa == "Ser" & depth$reconnect_eps == 4, ][1, ]
cug_row <- depth[grepl("CUG clade", depth$lineage) &
                   depth$table_id == 26, ][1, ]
chl_row <- depth[grepl("Chlorophyceae", depth$lineage) &
                   depth$table_id == 16, ][1, ]

fmt_age <- function(mya) {
  if (mya >= 1000) sprintf("~%.1f Gya", mya / 1000)
  else sprintf("~%d Mya", round(mya))
}
annot_df <- data.frame(
  x = c(ser_row$age_midpoint_mya,
        cug_row$age_midpoint_mya,
        chl_row$age_midpoint_mya),
  y = c(ser_row$reconnect_eps + 0.4,
        cug_row$reconnect_eps + 0.4,
        chl_row$reconnect_eps - 0.4),
  label = c(
    paste0("Serine\n(pre-LUCA)"),
    paste0("CUG clade\n(", fmt_age(cug_row$age_midpoint_mya), ")"),
    paste0("Chlorophyceae\n(", fmt_age(chl_row$age_midpoint_mya), ")")
  ),
...
p4 <- ggplot(depth, aes(x = age_midpoint_mya, y = reconnect_eps,
  ...
  labs(
    ...
    subtitle = bquote("Spearman" ~ rho ~ "=" ~
                      .(sprintf("%.2f", spearman_rho)) ~
                      "-- ordinal correspondence, not monotonic rank"),
...
# Round-2 audit fix (T17): compute subtitle counts from catalogue.csv rather
# than hard-coding "15 predictions: 9 verified, 5 tested, 1 null" — those
# literals silently drifted the moment claim_hierarchy changed. Include every
# status observed in the CSV so downstream category renames auto-propagate.
cat_status_counts <- table(cat_data$status)
cat_n_total       <- nrow(cat_data)
# Preferred display order for the subtitle; unknown labels sort last alphabetically.
status_display_order <- c("verified", "tested", "pending", "null",
                          "tautological", "exploratory")
status_display_labels <- c(verified = "verified", tested = "tested",
                           pending = "pending", null = "falsified",
                           tautological = "tautological",
                           exploratory = "exploratory")
present_status <- names(cat_status_counts)
ordered_status <- c(intersect(status_display_order, present_status),
                    sort(setdiff(present_status, status_display_order)))
subtitle_parts <- vapply(ordered_status, function(s) {
  label <- if (!is.na(status_display_labels[s])) status_display_labels[[s]] else s
  paste0(cat_status_counts[[s]], " ", label)
}, character(1))
catalogue_subtitle <- paste0(cat_n_total, " predictions: ",
