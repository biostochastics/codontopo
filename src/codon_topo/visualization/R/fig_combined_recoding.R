...
fig_a <- ggplot(ser_ala_data, aes(x = amino_acid, y = delta_local, fill = amino_acid)) +
  ...
  # Round-2 audit fix (T17): panels A/B/C use ILLUSTRATIVE simulated data
  # (see file header) — banner marks them SCHEMATIC so readers cannot
  # mistake the sim distributions for measured values. Data-driving from
  # CodonSafe CSVs was explicitly deferred by the remediation plan.
  annotate("label", x = 0.5, y = Inf, label = "SCHEMATIC",
           hjust = 0, vjust = 1.3, size = 3.2, fontface = "bold",
           color = "grey20", fill = alpha("#FFF3B0", 0.95)) +
  labs(
...
fig_b <- ggplot(gene_data, aes(x = gene_type, y = abs_log2fc, fill = gene_type)) +
  ...
  # SCHEMATIC banner (T17) — see Panel A comment for rationale.
  annotate("label", x = 0.5, y = Inf, label = "SCHEMATIC",
           hjust = 0, vjust = 1.3, size = 3.2, fontface = "bold",
           color = "grey20", fill = alpha("#FFF3B0", 0.95)) +
  labs(
...
fig_c <- ggplot(srz_data, aes(x = delta_local,
  ...
  # SCHEMATIC banner (T17) — see Panel A comment for rationale.
  annotate("label", x = -Inf, y = Inf, label = "SCHEMATIC",
           hjust = -0.05, vjust = 1.3, size = 3.2, fontface = "bold",
           color = "grey20", fill = alpha("#FFF3B0", 0.95)) +
  labs(
