...
# Face vector aligned with the sorted factor levels so the default encoding's
# y-axis tick is drawn bold (readers are directed to it from the caption).
default_label <- as.character(df$encoding_label[df$encoding_index == 0])
lvls <- levels(df$encoding_label)
tick_face <- ifelse(lvls == default_label, "bold", "plain")

# Placement for the "default" callout: just past the default bar's p-value label
# (bar end is ~3.4, y-axis expands to ~4.0), so the annotation sits inside the
# plot area without colliding with the p-value text.
default_depletion <- df$depletion_fold[df$encoding_index == 0]
annot_y <- default_depletion + 0.35

# Round-2 audit fix (T17): compute the "N/24 give no significant depletion"
# count inline from the JSON rather than hard-coding "8/24" — if the sweep
# expands or the encoding set changes, the subtitle now updates automatically.
n_encodings   <- nrow(df)
n_nonsig      <- sum(!df$signif)
sweep_subtitle <- sprintf(
  "%d/%d encodings give no significant depletion; H(3,4) is encoding-independent and is the primary test",
  n_nonsig, n_encodings
)
...
p1 <- ggplot(df, aes(x = encoding_label, y = depletion_fold, fill = signif)) +
  ...
  annotate("text", x = default_label, y = annot_y,
           label = "<- default", hjust = 0, size = 3.0,
           fontface = "italic", family = "sans") +
  scale_fill_manual(values = c("FALSE" = "grey80", "TRUE" = "#3182bd"),
                    labels = c("FALSE" = "p >= 0.05", "TRUE" = "p < 0.05"),
                    name   = NULL) +
  scale_y_continuous(expand = expansion(mult = c(0, 0.18))) +
  labs(x = "Base-to-bit encoding (sorted by Q_6 depletion fold; default in bold)",
  ...
  theme(axis.text.y = element_text(face = tick_face))
