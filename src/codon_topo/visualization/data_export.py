...
def export_disconnection_catalogue(
    ...
    Every one of the 27 NCBI translation tables must appear at least once
    in the output (Fig 1B and FigB depend on this invariant so their axes
    render all tables). In practice this holds because Serine is topologically
    disconnected at epsilon=1 under the default encoding in every known
    genetic code — one of the SUPPORTED claims in the manuscript. We assert
    the invariant explicitly so a future change (new encoding, new NCBI
    table, altered ``disconnection_catalogue`` semantics) fails loudly here
    rather than silently dropping tables from downstream figures.
    """
    output = Path(output_path)
    rows = []
    ...
    expected_tables = set(all_table_ids())
    missing = expected_tables - seen_tables
    if missing:
        raise RuntimeError(
            "export_disconnection_catalogue dropped NCBI translation tables "
            f"{sorted(missing)}: no disconnection catalogue entries were "
            "returned for these tables under the default encoding. This "
            "breaks the 'all 27 NCBI tables' invariant relied on by Fig 1B "
            "(main text) and FigB (supplement). Investigate "
            "codon_topo.core.homology.disconnection_catalogue and the "
            "default encoding before regenerating downstream figures."
        )
