...
class TestClaimHierarchy:
    ...
    def test_trna_is_exploratory_not_supported(self):
        """tRNA enrichment: median MIS Stouffer p just clears 0.05 but the
        worst-case subset does not and the topology-breaking subset is null;
        the claim is exploratory, not primary."""
        exp = exploratory_claims()
        assert any(c.id == "trna_enrichment_reassigned_aa" for c in exp)
        sup = supported_claims()
